...
size_t physics_body_create(vec2 position, vec2 size){
    ...
    if (array_list_append(state.body_list, &body) == (size_t)-1) ERROR_EXIT("Could not append body to list\n");
...
size_t physics_static_body_create(vec2 position, vec2 size){
    ...
    if (array_list_append(state.static_body_list, &body) == (size_t)-1) ERROR_EXIT("Could not append body to list\n");
