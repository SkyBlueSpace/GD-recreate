...
size_t array_list_append(Array_List* list, void* item){
    if (list->len == list->capacity){
        ...
        if (!items) ERROR_RETURN(-1, "Could not allocate memory for Array_List\n");
        list->items = items;
    }
...
void* array_list_get(Array_List* list, size_t index){
    if (index >= list->len) ERROR_RETURN(NULL, "Index out of bounds\n");
    return (u8*)list->items + index * list->item_size;
}

u8 array_list_remove(Array_List* list, size_t index){
    if (list->len == 0) ERROR_RETURN(1, "List is empty\n");
    if (index >= list->len) ERROR_RETURN(1, "Index out of bounds\n");
    if (list->len == 1){list->len = 0; return 0;}
    --list->len;
