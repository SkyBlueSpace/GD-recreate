...
static void input_handle(Body* body_player){
	if (global.input.escape > 0) {should_quit = true;}
	...
	if (global.input.left > 0) velx -= 1000;
	if (global.input.right >0) velx += 1000;
	if (global.input.up > 0) vely = 2000;
	if (global.input.down > 0) vely -= 800;
...
int main(int argc, char* argv[]){
	...
	u32 body_id = physics_body_create((vec2){100, 100}, (vec2){50,50});
	...
	while (!should_quit){
		...
		render_aabb((f32*)body_player, GREEN);
